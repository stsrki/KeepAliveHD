    ...
    public partial class Main : Form
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            try
            {
                ...
                InitialiseTimers( ApplicationConfiguration.WritingEnabled );
        ...
        private void ManageDriveOperation( Database.DriveInfo driveInfo )
        {
            ...
            if ( Directory.Exists( driveInfo.Drive ) && ( ignoreVolumeNames == true || ( ignoreVolumeNames == false && driveInfo.VolumeNames.Contains( volumeName ) ) ) )
            {
                try
                {
                    ...
                    {
                        Directory.GetFiles( driveInfo.Drive, "*.*" ).Take( 5 ).ToList();
                    }
                    else
                    {
                        string sFileName = Path.Combine( Path.GetFullPath( driveInfo.Drive ), "KeepAliveHD.txt" );

                        using ( StreamWriter writer = new StreamWriter( sFileName, false, Encoding.ASCII ) )
                        {
        ...
        private void dgDrives_SelectionChanged( object sender, EventArgs e )
        {
            ...
            else
            {
                ...
                if ( row.Cells[DriveStatus.Index].Value.ToString() == "0" )
                    ...
                    row.DefaultCellStyle.SelectionForeColor = ApplicationConfiguration.WritingEnabled ? Color.Green : Color.Blue;
            }
        ...
        private void dgDrives_RowsAdded( object sender, DataGridViewRowsAddedEventArgs e )
        {
            foreach ( DataGridViewRow row in dgDrives.Rows )
            {
                if ( row.Cells[DriveStatus.Index].Value.ToString() == "0" )
                    ...
                    row.DefaultCellStyle.ForeColor = ApplicationConfiguration.WritingEnabled ? Color.Green : Color.Blue;
            }
        }
        ...
        {
            btnEngineStart.Enabled = false;
            btnEngineStop.Enabled = true;
        ...
        {
            btnEngineStart.Enabled = true;
            btnEngineStop.Enabled = false;
        ...
        {
            Database.DatabaseManager.FillDrives( dgDrives, ApplicationConfiguration.WritingEnabled );
        ...
        private void FillSettings()
        {
            ...
            btnEngineStart.Enabled = !ApplicationConfiguration.WritingEnabled;
            btnEngineStop.Enabled = ApplicationConfiguration.WritingEnabled;
        ...
        private void ManageDrivesNew()
        {
            try
            {
                using ( DriveSettings form = new DriveSettings( 0 ) )
                {
        ...
        private void ManageDrivesNewMulti()
        {
            try
            {
                if ( dgConnectedDrives.SelectedRows.Count > 0 )
                {
                    ...
                    using ( DriveSettingsMulti form = new DriveSettingsMulti( drives ) )
                    {
                        if ( form.ShowDialog() == DialogResult.OK )
                        {
                            InitialiseTimers( ApplicationConfiguration.WritingEnabled );
        ...
        private void ManageDrivesEdit()
        {
            try
            {
                if ( dgDrives.SelectedRows.Count == 1 )
                {
                    ...
                    using ( DriveSettings form = new DriveSettings( id ) )
                    {
                        if ( form.ShowDialog() == DialogResult.OK )
                        {
                            InitialiseTimers( ApplicationConfiguration.WritingEnabled );
        ...
        private void ManageVolumeNamesEdit()
        {
            try
            {
                if ( dgDrives.SelectedRows.Count > 0 )
                {
                    ...
                    using ( VolumeNames form = new VolumeNames( id ) )
                    {
                        if ( form.ShowDialog() == DialogResult.OK )
                        {
                            InitialiseTimers( ApplicationConfiguration.WritingEnabled );
